...
import (
	...
	"github.com/cockroachdb/dev/io/exec"
	"github.com/cockroachdb/dev/io/os"
...
func makeDevCmd() *dev {
	var dev dev
	dev.log = log.New(ioutil.Discard, "DEBUG: ", 0) // used for debug logging (see --debug)
	dev.exec = exec.New(exec.WithLogger(dev.log))
	dev.os = os.New(os.WithLogger(dev.log))

	dev.cli = &cobra.Command{
...
`,
	...
	dev.cli.AddCommand(
		makeBenchCmd(dev.bench),
		makeBuildCmd(dev.build),
		makeGenerateCmd(dev.generate),
		makeLintCmd(dev.lint),
		makeTestCmd(dev.test),
	)

	// Add all the shared flags.
	var debugVar bool
	for _, subCmd := range dev.cli.Commands() {
	...
	for _, subCmd := range dev.cli.Commands() {
		subCmd.PreRun = func(cmd *cobra.Command, args []string) {
			if debugVar {
				dev.log.SetOutput(stdos.Stderr)
			}
		}
	}

	// Hide the `help` sub-command.
	dev.cli.SetHelpCommand(&cobra.Command{
		Use:    "noop-help",
		Hidden: true,
	})
